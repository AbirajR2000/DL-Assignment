...
if task == "Sentiment Classification":
    ...
    if new_review_text.strip():
        ...
        if model_option == "DeepLearning":
            with open('spampercepton.pkl', 'rb') as file:
                model = pickle.load(file)
        elif model_option == "Backpropagation":
            with open('spambackpropogation.pkl', 'rb') as file:
                model = pickle.load(file)
        elif model_option == "DNN":
            model = load_model('spam_dnn_model.pkl')
        elif model_option == "RNN":
            model = load_model('RNN.pkl')
        elif model_option == "LSTM":
            model = load_model('LSTM.pkl')
    ...
    if uploaded_file is not None:
        ...
        model = load_model('CNN_tumor.pkl')
